  ...
  methods: {
      ...
      queryDataByUser(queryData).then((res) => {
        if (res.data.code === '200') {
          ...
          queryLists.forEach((item) => {
            item.mainTitle = this.changeColorMainTitle(item.mainTitle)
    ...
    changeColor(result) {
      ...
      if (this.text) {
        ...
        let replaceString = `<span style="color: #ed4014;font-weight: bold;font-size: 25px">${text}</span>`
        result = result.replace(replaceReg, replaceString)
      }
      return result
    },
    changeColorMainTitle(result) {
      //result为接口返回的数据
      if (this.text) {
        /**
         * 使用正则表达式进行全文匹配关键词
